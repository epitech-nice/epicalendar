...
import { OpeningRequestsService, OpeningRequest } from '@/services/openingRequestsService'
...
export default function ManageOpeningRequestsDisplayId() {
    ...
    const handleDeleteOpeningRequest = async (openingRequestId: string) => {
        console.log('Attempting to delete opening request with ID:', openingRequestId);

        if (!confirm('Are you sure you want to delete this opening request? This action cannot be undone.')) {
            return;
        }
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'An error occurred while deleting the opening request.')
        }
    };
    ...
    useEffect(() => {
        ...
        if (openingRequest !== null && user.role === 'student' && openingRequest.account !== user.email) {
            setError('You do not have permission to access this page.');
            return;
        }
    }, [fetchOpeningRequest, id, isAuthenticated, loading, openingRequest, router, user])
    ...
    if (loading) {
        ...
        content = openingRequest ? (
            <div>
                ...
                <div>
                    {user?.role !== 'student' && (
                        <button onClick={() => router.push(`/opening-requests/edit/${openingRequest?._id}`)}>
                            Edit
                        </button>
                    )}

                    <button onClick={() => handleDeleteOpeningRequest(openingRequest._id!)}>
                        Delete
                    </button>
                </div>
                ...
                <div>
                    <b>Campus opens at:</b>
                    {openingRequest.open ? new Date(openingRequest.open).toLocaleTimeString('fr-FR', {
                        hour: '2-digit',
                        minute: '2-digit'
                    }) : 'Not set'}
                    <b>Campus closes at:</b>
                    {openingRequest.close ? new Date(openingRequest.close).toLocaleTimeString('fr-FR', {
                        hour: '2-digit',
                        minute: '2-digit'
                    }) : 'Not set'}
                </div>
