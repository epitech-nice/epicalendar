...
export default function ManageOpeningRequests() {
    ...
    const fetchOpeningRequests = useCallback(async () => {
        try {
            const fetchedOpeningRequests = await OpeningRequestsService.getOpeningRequests();
            setOpeningRequests(fetchedOpeningRequests);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'An error occurred while fetching opening requests.');
        }
    }, []);

    const handleDeleteOpeningRequests = async (openingRequestsId: string) => {
        console.log('Attempting to delete opening requests with ID:', openingRequestsId);
        ...
        try {
            await OpeningRequestsService.deleteOpeningRequest(openingRequestsId);
            ...
            <div>
                ...
                <table>
                    ...
                    {openingRequests.map(openingRequest => (
                        <tr key={openingRequest._id} onClick={() => router.push(`/opening-requests/display/${openingRequest._id}`)} style={{ cursor: 'pointer' }}>
                            <td>
